...
int main() {
	...
	chan = iio_device_get_channel(dev, 3);
    ...
    iio_channel_attr_read_longlong(chan, "raw", &raw);
    iio_channel_attr_read_longlong(chan, "offset", &offset);
    iio_channel_attr_read_double(chan, "scale", &scale);

    double temp = (raw + offset) * scale;
    printf("Temperature: %f\n", temp);

	//iio_channel_attr_write(chan, chan_attr, "87");
	//iio_channel_attr_read(chan, chan_attr, valoare, 16);
	printf("%s : %s\n", chan_id, chan_attr);
	//printf("%s\n", valoare);
