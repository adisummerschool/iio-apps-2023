...
int main()
{
	...
	while(axa_x > 0.1 && axa_x < -0.1)
	{
	if(axa_x > 0.1)
	{
		printf("Regleaza POT X spre stanga!\n");
		sleep(1);
	...
	if(axa_x < -0.1)
	{
		printf("Regleaza POT X spre dreapta!\n");
		sleep(1);
	}
	else break;
	}}

	while(axa_y > 0.1 && axa_y < -0.1)
	{
	if(axa_y > 0.1)
	{
		printf("Regleaza POT Y spre stanga!\n");
		sleep(1);
	}
	else{
	if(axa_y < -0.1)
	{	
		printf("Regleaza POT Y spre dreapta!");
		sleep(1);
	}else break;
	}}


	while(axa_z > 1.65 && axa_x < 1.55)
	{
	if(axa_x > 1.65)
	{
		printf("Regleaza POT Z spre stanga!");
		sleep(1);
	} else {
	if(axa_x < 1.55)
	{	
		printf("Regleaza POT Z spre dreapta!");
		sleep(1);
	}else break;
	}}
