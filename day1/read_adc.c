...
int main() {
	...
	for(int i=0; i<chan_cnt; i++){
		...
		iio_channel_attr_read_longlong(chan, chan_attr, &adc_chan_nr);

		adc_chan_voltage = (double)adc_chan_nr * (2.5 / 4096);
		adc_chan_acceleration = (double)adc_chan_nr / 4096 * 1000;

		printf("%s : %s :", chan_id, channels[i]);
		printf("%d\t%.2fV\t%.2fmg\n", adc_chan_nr, adc_chan_voltage, adc_chan_acceleration);
	}
